...
{
    public partial class CalculateResultPreview : Window
    {
        private long _agentId;
        private string _yearMonth = string.Empty;
        private RateMonthlyDetail? _currentData;

        // 保存成功事件
        public event EventHandler? SaveSuccess;

        public CalculateResultPreview()
        {
            InitializeComponent();
        }
        ...
        private async System.Threading.Tasks.Task LoadDataAsync()
        {
            try
            {
                ...
                var response = await NewApiClient.PostAsync<RateMonthlyDetail>(
                    "/rate/detail/calculateCvp",
                    requestDto);
        ...
        private async void ApplyButton_Click(object? sender, RoutedEventArgs e)
        {
            try
            {
                ...
                var requestDto = new CvpCalculateDto
                {
                    AgentId = _agentId,
                    YearMonth = _yearMonth,
                    RateMonthlyDetail = _currentData
                };
