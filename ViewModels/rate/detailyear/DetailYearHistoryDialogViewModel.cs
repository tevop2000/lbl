...
{
    public partial class DetailYearHistoryDialogViewModel : ObservableObject
    {
        private readonly long _agentId;
        private readonly string _yearMonth;
        private readonly long _versionId;

        public DetailYearHistoryDialogViewModel(long agentId, string yearMonth, long versionId)
        {
            _agentId = agentId;
        ...
        public async Task LoadDataAsync()
        {
            ...
            {
                Logger.Error($"加载年度方案详情失败: {ex.Message}");
            }
            finally
            {
                IsLoading = false;
        ...
        {
            var url = $"/rate/detailyearhistory/getRateMonthlyDetailYearHistory?agentId={_agentId}&yearMonth={_yearMonth}&versionId={_versionId}";
        ...
        private void AddMonthlyDataItem(JObject data)
        {
            ...
            var month = GetSafeValue<int>(data, "yearMonth", 0);
        ...
        {
            var url = $"/rate/saletargetyearhistory/getByAgentIdAndYearMonth?agentId={_agentId}&yearMonth={_yearMonth}&versionId={_versionId}";
            var response = await NewApiClient.GetAsync<dynamic>(url);
        ...
        {
            var url = $"/rate/expenseyearhistory/getByAgentIdAndYearMonth?agentId={_agentId}&yearMonth={_yearMonth}&versionId={_versionId}";
            var response = await NewApiClient.GetAsync<dynamic>(url);
        ...
        {
            var url = $"/rate/structureyearhistory/getByAgentIdAndYearMonth?agentId={_agentId}&yearMonth={_yearMonth}&versionId={_versionId}";
            var response = await NewApiClient.GetAsync<dynamic>(url);
            ...
            if (response.Code == 200 && response.Data != null)
            {
                ...
                if (jArray != null)
                {
                    ...
                    foreach (var item in jArray)
                    {
                        ...
                        if (obj != null)
                        {
                            var product = new ProductStructureDto
                            {
                                StructureId = GetSafeValue<long>(obj, "structureId", 0),
                                ModelId = GetSafeValue<long>(obj, "modelId", 0),
                                ItemModel = obj["itemModel"]?.ToString() ?? "",
                                AgentId = GetSafeValue<long>(obj, "agentId", 0),
                                AgentName = obj["agentName"]?.ToString() ?? "",
                                YearMonth = obj["yearMonth"]?.ToString() ?? "",
                                StructureRatio = GetSafeValue<decimal>(obj, "structureRatio", 0),
                                RemiumPrice = GetSafeValue<decimal>(obj, "remiumPrice", 0),
                                Commission = GetSafeValue<decimal>(obj, "commission", 0),
                                RemiumCost = GetSafeValue<decimal>(obj, "remiumCost", 0),
                                PremiumDiscount = GetSafeValue<decimal>(obj, "premiumDiscount", 0),
                                PriceAdjustment = GetSafeValue<decimal>(obj, "priceAdjustment", 0)
                            };
                            ProductStructures.Add(product);
                        }
                    }
                }
            }
    ...
    public class MonthlyDataItem
    {
        ...
        public double BreakEvenRate { get; set; }
        public double BreakEvenRatePercent { get; set; }
        public double OptimizedSales { get; set; }
        public double SalesGrowthProfit { get; set; }
        public double StructureOptimizeProfit { get; set; }
        public double PremiumProfit { get; set; }
        public double TotalExtraProfit { get; set; }
        public double AdjustedNetProfit { get; set; }
    }
}